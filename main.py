...
def main():
    ...
    print(f"Combined GeoJSON saved to {output_geojson_path}")

    # Step 5: Simplify evacuation routes graph
    simplified_graph = create_and_simplify_graph(routes_geojson_path)
    print(f"Simplified graph has {simplified_graph.number_of_nodes()} nodes and {simplified_graph.number_of_edges()} edges.")

    # Step 6: Visualize population and evacuation routes
    plot_population_and_routes(combined_gdf, routes_gdf)
