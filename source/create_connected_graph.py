...
def process_all_centroids(G, centroids_gdf, output_dir):
    """Process all centroids, find the shortest path to nearest sink for each."""
    ...
    for _, centroid in centroids_gdf.iterrows():
        ...
        # Find the shortest path to the closest sink
        shortest_path = None
            ...
            for sink_node in sink_nodes:
                try:
                    ...
                    if distance < shortest_distance:
                        ...
                        closest_sink = G.nodes[sink_node].get("zip_code")
        ...
        # Report results and visualize
        if shortest_path:
            print(f"Shortest path from centroid {zip_code} to sink {closest_sink} found!")
            print(f"Path distance: {shortest_distance:.2f} meters ({shortest_distance/1000:.2f} km)")
            ...
            print("Road types used in evacuation path:")
            for road_type, count in road_types_in_path.items():
            ...
            # Visualize the path
            output_path = os.path.join(output_dir, f"path_{zip_code}_to_{closest_sink}.png")
            visualize_path_with_road_types(G, shortest_path, output_path, zip_code)
    ...
    with open(summary_path, 'w') as f:
        ...
        for result in results:
            ...
            if result['sink']:
                f.write(f"Closest Sink: {result['sink']}\n")
                f.write(f"Distance: {result['distance_km']:.2f} km ({result['distance_meters']:.2f} m)\n")
                f.write(f"Path Nodes: {result['path_nodes']}\n")
                
                if result['road_types']:
                    f.write("Road Types Used:\n")
                    for road_type, count in result['road_types'].items():
